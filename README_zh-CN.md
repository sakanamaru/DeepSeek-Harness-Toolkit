...
- 🔧 **安装 / 修复 / 更新** — 默认官方源 npmjs.org，国内镜像可选手动；失败自动换另一源重试，不污染全局 npm 配置；安装/更新可选历史版本列表
...
| 更新 dsh | 菜单 8：检测新版本 → 选版本（回车最新 / L 历史列表）→ ⚠️ 破坏性警告**双确认** → 更新前自动备份（`-pre-update` + 旧版本号）→ npm 安装 → 记录本机历史版本（最多 10 个，列表带 `*`）；dsh 运行中拒绝；失败可从备份恢复 + 按版本回滚 |
...
- **集成测试（打桩端到端矩阵）**：**25 个用例**，变体 A/C 自动打桩，真实探测 3080（覆盖保留策略、运行中禁止恢复/导入、双语断言等）：
  ```
