...
- 🌍 **单 exe 双语界面** — 简体中文 / English，基于 .NET Framework 4.x（Win10/11 自带），无需额外安装；**启动静默检查更新**（GitHub Releases，`check_update=off` 可关）；**1MB 日志轮转**（`launcher.log → launcher.log.1`）
...
| 更新检查 | 启动后静默查询 GitHub Releases API，仅当存在新版本时提示（附下载链接）；离线/接口失败静默；`check_update=off` 关闭 |
...
- **单实例锁改为产品级固定名**（`DeepSeek-Harness-Toolkit-single`）— v2.0 与 v2.1 无法在同一机器同时打开两个交互实例
