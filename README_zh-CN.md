...
**三个版本共有的能力**：三页界面（首页状态灯 + dsh 版本 + Web 地址 + 操作按钮 · 日志 · 关于）、深/浅主题、中英双语、圆角无边框、logo 内嵌；操作：安装 / 启动 Web / 停止服务 / 立即备份 / **恢复备份（弹选择框自选备份文件夹 + 确认）** / 检查更新 / 卸载 / 桌面快捷方式 / 刷新状态。服务已运行时点「启动 Web」直接打开浏览器；恢复前自动备份当前数据兜底。
