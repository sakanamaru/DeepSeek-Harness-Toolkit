...
- 🔧 **安装 / 修复 / 更新** — 默认官方源 npmjs.org，国内镜像可选手动；失败自动换另一源重试，不污染全局 npm 配置；安装/更新可选历史版本列表（v2.1.1）
- 📊 **智能启动 + 三态监控** — 端口＋HTTP 双重校验的**运行中 / 启动中(未就绪) / 已停止**三态（其他程序占用 3080 不再误判为 dsh）；每 3 秒自动刷新，掉线红字提醒
- 💾 **备份 / 恢复 + 保留策略** — 一键备份到 `backup\`，支持**多工作区**与**跨电脑导入**，长路径安全；**手动备份永久保留**，自动/保护类备份自动清理（最多保留 N 份、保底 3 份、`keep_backups=` 可配）；dsh 运行中**禁止恢复/导入**
- 🛡️ **清除数据三重防护** — 两步确认（日期 + yes）且**清除前自动备份**；Web 服务运行中禁止清除；随包分发的根目录标记让"单独复制的 exe"永久无法清除
- 🌍 **单 exe 双语界面** — 简体中文 / English，基于 .NET Framework 4.x（Win10/11 自带），无需额外安装；**启动静默检查更新**（GitHub Releases，`check_update=off` 可关；dsh 本体更新检测 `check_dsh_update=off` 可关）；**1MB 日志轮转**（`launcher.log → launcher.log.1`）
...
| 安装 / 修复 | 安装时询问源：**默认官方源 npmjs.org**，国内镜像 npmmirror 可选；失败自动换另一源重试，不污染全局 npm 配置；回车=装最新版，L=查看历史版本列表可选装（v2.1.1） |
| 状态监控 | 每 3 秒自动刷新服务状态（三态）/端口/运行时长，服务掉线红字提醒；按 1 返回 / 2 打开 WebUI |
| 备份 / 恢复 | 一键备份数据目录到 `backup\`（可添加**多个工作区**：自动探测并校验，之后逐个输入路径、留空结束，备份包按 `_workspace\名称\` 分包存放并可逐一恢复），自动跳过 node_modules 与自身备份目录；支持列表恢复、跨电脑导入、直接打开备份文件夹；**手动备份（无后缀）永久保留**，自动/保护类备份（`-auto / -pre-*`）超出 `keep_backups`（默认 10、最小 3）按最旧自动清理；**dsh 运行中禁止恢复/导入**（与清除数据同一防线） |
...
| 更新 dsh | 菜单 8：检测新版本 → 选版本（回车最新 / L 历史列表）→ ⚠️ 破坏性警告**双确认** → 更新前自动备份（`-pre-update` + 旧版本号）→ npm 安装 → 记录本机历史版本（最多 10 个，列表带 `*`）；dsh 运行中拒绝；失败可从备份恢复 + 按版本回滚（v2.1.1） |
...
- **备份保留策略** — 无后缀备份（手动）**永久保留**；自动 / 保护类备份（`-auto`、`-pre-restore`、`-pre-import`、`-pre-wipe`）超出 `keep_backups`（默认 10、最小 3）按最旧清理，删除前打印名单
- **服务三态检测** — 端口开＋HTTP 2xx/3xx＝**运行中**；仅端口＝**启动中（未就绪）**；全无＝**已停止**（启动等待、监控、`check`、开浏览器全部接入）
...
- **dsh 本体更新管理** — 菜单 8「更新 dsh」：npm 最新版检测 + 历史版本列表（支持 rc 预发布版本）+ 更新前自动备份（`-pre-update` + 旧版本号）＋ ⚠️ 破坏性警告**双确认**；dsh 运行中拒绝；安装/更新后记录本机历史版本（最多 10 个，列表带 `*` 标记）；`check` 显示 dsh 最新版本
- **安装可选版本** — 安装时回车=最新版，或 `L` 查看历史版本列表
- 配置新增 `check_dsh_update=off` 关闭 dsh 更新检测
- **保护性备份严格化** — Pre-清除 / Pre-恢复 / Pre-导入 / Pre-更新 备份遇到**任意文件复制失败**（被锁/损坏）即整体失败、**中止后续危险操作**（不再用有缺漏的安全网继续）；手动/自动备份保持 best-effort（跳过并记日志）
- **dsh 最新版本合法性校验** — `npm view` 结果按「干净的核心段」校验（接受 `0.1.1-rc.2` 这类预发布版本，拒绝垃圾值），非法视为离线
...
- **单元测试（同程序集测试代理）**：`/define:UNIT` 构建，测试入口在 `tests\unit_tests.cs`，被测的是生产代码本体：
  ...
  unittests.exe
  ```
  退出码 0=全过。覆盖：路径往返（含 UNC / 中文空格）、工作区黑名单（19 拒绝 + 3 允许）、dsh 数据目录标记、日志轮转、备份命名 + 保留策略、服务三态判定、版本比较 / 发布解析 / 更新探测。

- **集成测试（打桩端到端矩阵）**：**22 个用例**，变体 A/C 自动打桩，真实探测 3080（覆盖保留策略、运行中禁止恢复/导入、双语断言等）：
  ```
